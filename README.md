...
1. Загрузить датасет.
2. Запустить файл ***start_project.bat***, дождаться загрузки и нажать Enter.
...
Для этого создан файл *scripts/prepare_dataset.py*, который берёт содержимое папки *'data/input/raw'* в форматах *'.jpg'*, *'.jpeg'*, *'.png'*, *'.bmp'* и через модель yolov8m.pt находит все подходящие прямоугольные области на изображении и записывает результат в YOLO формате в файлы по пути *'data/input/labels'*.

Для детекции были использованы следующие промпты:
* Позитивные:
...
Так же полученная разметка была разбита на обучающую (*train*) и валидационную (*val*) выборки.
<br><br/>
